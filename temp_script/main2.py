...
dump1_path='/home/zzg/Documents/cacheFCN1113.pkl'


dump_path = '/home/zzg/Documents/DAN/dataset/src/CacheFCN1113.pkl'
XTrain_path = '/home/zzg/Documents' + '/XTraincache.pkl'
YTrain_path = '/home/zzg/Documents' + '/YTraincache.pkl'
XTest_path = '/home/zzg/Documents/XTestcache.pkl'
YTest_path = '/home/zzg/Documents/YTestcache.pkl'
...
# 输入一个x返回output
tf_x_src = tf.placeholder(tf.float32, [None, 6], name="src_placeholder_x")  # input x
tf_x_tgt = tf.placeholder(tf.float32, [None, 6], name="tgt_placeholder_x")  # input x


tf_y_src = tf.placeholder(tf.float32, [None, 1], name="src_placeholder_y")  # input y
tf_y_tgt = tf.placeholder(tf.float32, [None, 1], name="tgt_placeholder_y")  # input y

# neural network layers
## 后续需改成两个with加一个整和 ====代码优化
l1 = tf.layers.dense(tf_x_src, 512, tf.nn.relu, name="l1")  # hidden layer
print('l1 ', l1.name)
l2 = tf.layers.dense(l1, 512, tf.nn.relu, name="l2")  # hidden layer
l3 = tf.layers.dense(l2, 512, tf.nn.relu, name="l3")  # hidden layer
l4 = tf.layers.dense(l3, 512, tf.nn.relu, name="l4")  # hidden layer
l5 = tf.layers.dense(l4, 512, tf.nn.relu, name="l5")  # hidden layer
l6 = tf.layers.dense(l5, 512, tf.nn.relu, name="l6")  # hidden layer
l7 = tf.layers.dense(l6, 512, tf.nn.relu, name="l7")  # hidden layer
l8 = tf.layers.dense(l7, 1, name="l8")

# output = tf.layers.dense(l7, 1, name="outputtttttttt_node")                     # output layer
Uui = tf.add(l8, l8)
output = tf.subtract(Uui, l8, name="output_node")
loss = tf.losses.mean_squared_error(tf_y_src, output)  # compute cost
optimizer=tf.train.GradientDescentOptimizer(0.0001)
train_op = optimizer.minimize(loss)
saveryy = tf.compat.v1.train.Saver()  # control training and others
